import type { StblDto } from "../shared";
 ...
 */
export default function writeStbl(stbl: StblDto): Buffer {
  ...
  encoder.uint16(stbl.header?.version ?? 5); // mnVersion (should be 5)
  encoder.uint8(stbl.header?.compressed ?? 0); // mnCompressed (should be null)
  encoder.uint64(stbl.entries.length); // mnNumEntries
  encoder.uint8(stbl.header?.reserved1 ?? 0); // mReserved[0] (should be null)
  encoder.uint8(stbl.header?.reserved2 ?? 0); // mReserved[1] (should be null)
